...
proc_create(char *name) {
	...
	strncpy(proc->p_comm, name,
			(PROC_NAME_LEN - 1) < strlen(name) ?
			PROC_NAME_LEN - 1 :
													strlen(name));
	proc->p_comm[PROC_NAME_LEN - 1] = '\0'; // If the source string is longer than num characters, only the first num characters will be copied to the destination string, and the destination string will not be null-terminated if the source string does not contain a null character within the first num characters.
	...
	// special process for init and idle
	if (proc->p_pid == PID_IDLE) {
		proc->p_pproc = NULL;
		dbg(DBG_PRINT, "(GRADING1A)\n");
	}
	//if (proc -> p_pid == PID_INIT) {proc -> p_pproc = (struct proc *) _proc_list.l_next;}
	//if(proc->p_pid == PID_INIT) proc->p_pproc = list_head(&_proc_list, proc_t, p_list_link);
	// add this new process to the parent's child proc list
	if (curproc != NULL) {
	...
	// put process into the global proc_list using handle, link
...
void proc_cleanup(int status) {
	...
	//make sure the parent process of current process is not waiting
	...
	if(curproc->p_pid != 2) vput(curproc->p_cwd);
	dbg(DBG_PRINT, "(GRADING2A)\n");
#endif
...
void proc_kill(proc_t *p, int status) {
	...
	dbg(DBG_PRINT, "(GRADING1C)\n");
	return kthread_cancel(list_head(&p->p_threads, kthread_t, kt_plink),
			(void *) status);
	...
	list_iterate_begin(&_proc_list,p,proc_t,p_list_link)
				{
					if (p->p_pid != PID_IDLE && p->p_pid != PID_INIT
							&& p != curproc&& p->p_pproc->p_pid != PID_IDLE) {
...
void proc_thread_exited(void *retval) {
	...
	dbg(DBG_PRINT, "(GRADING1A)\n");
	sched_switch();
	panic("\nReturned to proc_thread_exited()!!!\n");
}
