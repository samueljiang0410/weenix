...
kthread_create(struct proc *p, kthread_func_t func, long arg1, void *arg2)
{
        ...
        // fill in kthread_t data structure
        char * kstack = alloc_stack();
        context_t kthread_context;
        context_setup(&kthread_context, func, arg1, arg2, kstack, DEFAULT_STACK_SIZE, p->p_pagedir);
        kthread->kt_ctx = kthread_context;
	      kthread->kt_kstack = kstack;
        kthread->kt_retval = 0;
        kthread->kt_errno = 0;
        kthread->kt_proc = p;
        kthread->kt_cancelled = 0;
        kthread->kt_wchan = NULL; // the Q that thread is waiting on
        kthread->kt_state = KT_RUN;
//        if(p->p_pid == PID_IDLE) {
//          kthread->kt_state = KT_RUN;
//          dbg(DBG_PRINT, "(GRADING1A)\n");
//        }
        list_init(&kthread->kt_qlink);
        list_init(&kthread->kt_plink);
        // add thread to p's p_threads
...
kthread_clone(kthread_t *thr)
{
        ...
        char * kstack = alloc_stack();
        context_t kthread_context;
        context_setup(&kthread_context, NULL, 0, NULL, kstack, DEFAULT_STACK_SIZE, thr->kt_proc->p_pagedir);
        newthr->kt_ctx = kthread_context;
