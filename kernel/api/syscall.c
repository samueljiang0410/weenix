...
sys_read(read_args_t *arg)
{
        ...
        int bytes = 0;
        while((uint32_t)bytes < kern_args.nbytes) {
                uint32_t read_bytes = MIN(kern_args.nbytes - bytes, PAGE_SIZE);

                int ret = do_read(kern_args.fd, buf, read_bytes);
                if(ret < 0) {
                        page_free(buf);
                        curthr->kt_errno = -ret;
                ...
                err = copy_to_user((void *)((uint32_t)arg->buf + bytes), buf, ret);
                KASSERT(0 == err);
                // if((err = copy_to_user((void *)((uint32_t)arg->buf + bytes), buf, ret)) < 0) {
                //         curthr->kt_errno = -err;
                //         page_free(buf);
                //         return -1;
                // }
...
sys_write(write_args_t *arg)
{
        ...
        int bytes = 0;
        while((uint32_t)bytes < kern_args.nbytes) {
                uint32_t write_bytes = MIN(kern_args.nbytes - bytes, PAGE_SIZE);

                err = copy_from_user(buf, (void *)((uint32_t)kern_args.buf + bytes), write_bytes);
                KASSERT(0 == err);
                ...
                int ret = do_write(kern_args.fd, buf, write_bytes);
                if(ret < 0) {
                        page_free(buf);
                        curthr->kt_errno = -ret;
                        dbg(DBG_PRINT, "(GRADING3D 1)\n");
                ...
                bytes += ret;
                KASSERT((uint32_t)ret == write_bytes);
                // if((uint32_t)ret < write_bytes) {
                //     break;
                // }
                dbg(DBG_PRINT, "(GRADING3A)\n");
        }
...
sys_getdents(getdents_args_t *arg)
{
        ...
        int bytes = 0;
        int ndent = kern_args.count / sizeof(dirent_t);
        for(int i = 0; i < ndent; i++) {
                int ret = do_getdent(kern_args.fd, &dent);
                if(ret < 0) {
                        curthr->kt_errno = -ret;
                        dbg(DBG_PRINT, "(GRADING3D 1)\n");
                        return -1;
                ...
                err = copy_to_user(kern_args.dirp + i, &dent, ret);
                KASSERT(0 == err);
                // if((err = copy_to_user(kern_args.dirp + i, &dent, ret)) < 0) {
                //         curthr->kt_errno = -err;
                //         return -1;
                // }
