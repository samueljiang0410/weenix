...
void handle_pagefault(uintptr_t vaddr, uint32_t cause)
{
        ...
        //check the vmarea permission
        if (vma == NULL)
        {
                dbg(DBG_PRINT, "(GRADING3D 2)\n");
                do_exit(EFAULT);
                panic("\nReturned from do_exit.\n");
        }
        ...
        if (!(vma->vma_prot & PROT_READ) && !((cause & FAULT_EXEC) || (cause & FAULT_WRITE)))
        {
                dbg(DBG_PRINT, "(GRADING3D 2)\n");
                do_exit(EFAULT);
                panic("\nReturned from do_exit.\n");
        ...
        KASSERT((vma->vma_prot & PROT_EXEC) || !(cause & FAULT_EXEC));
        ...
        int res = pframe_lookup(vma->vma_obj, vma->vma_off + ADDR_TO_PN(vaddr) - vma->vma_start, forwrite, &pf);
        if (res < 0)
        {
                dbg(DBG_PRINT, "(GRADING3D 2)\n");
                do_exit(EFAULT);
