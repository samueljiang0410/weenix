...
do_brk(void *addr, void **ret)
{
        ...
        if(NULL == addr || addr == curproc->p_brk) {
                *ret = curproc->p_brk;
                dbg(DBG_PRINT, "(GRADING3A)\n");
                return 0;
        }

        if((uint32_t)addr < (uint32_t)curproc->p_start_brk || (uint32_t)addr > (uint32_t)USER_MEM_HIGH) {
                dbg(DBG_PRINT, "(GRADING3D 2)\n");
                return -ENOMEM;
        }

        uint32_t brk_vfn = ADDR_TO_PN(PAGE_ALIGN_UP(addr));
        uint32_t old_brk_vfn = ADDR_TO_PN(PAGE_ALIGN_UP(curproc->p_brk));
        // if brk and old brk is not in the same page, modify vmmap
        if(brk_vfn < old_brk_vfn) {
                // brk < old brk
                int rm_result = vmmap_remove(curproc->p_vmmap, brk_vfn, old_brk_vfn - brk_vfn);
                KASSERT(rm_result >= 0);
                dbg(DBG_PRINT, "(GRADING3D 2)\n");
        }else if(brk_vfn > old_brk_vfn) {
                // brk > old brk
                //ckeck if the range is available
                if(!vmmap_is_range_empty(curproc->p_vmmap, old_brk_vfn, brk_vfn - old_brk_vfn)) {
                        dbg(DBG_PRINT, "(GRADING3D 2)\n");
                        return -ENOMEM;
                }

                // check if there is a vmarea, if not, create one
                uint32_t vma_start = ADDR_TO_PN(PAGE_ALIGN_UP(curproc->p_start_brk));
                vmarea_t *vma = vmmap_lookup(curproc->p_vmmap, vma_start);
                if(vma == NULL ) {
                        //create a vmarea for heap
                        int map_result = vmmap_map(curproc->p_vmmap, NULL, vma_start, brk_vfn - vma_start,
                                PROT_READ | PROT_WRITE, MAP_PRIVATE, 0, VMMAP_DIR_HILO,  &vma); 
                        KASSERT(0 == map_result);
                        // if(map_result < 0) {
                        //         return map_result;
                        // }
                        dbg(DBG_PRINT, "(GRADING3A)\n");
                }else {
                        // if vmarea exists, set vma_end
                        KASSERT(NULL != vma);
                        // if(vma == NULL) {
                        //         vma = vmmap_lookup(curproc->p_vmmap, ADDR_TO_PN(PAGE_ALIGN_DOWN(curproc->p_start_brk)));
                        // }
                        vma->vma_end = brk_vfn;
                        dbg(DBG_PRINT, "(GRADING3A)\n");
                }
