...
int do_mmap(void *addr, size_t len, int prot, int flags, int fd, off_t off,
	...
	if (len
			<= 0|| len >= (USER_MEM_HIGH - USER_MEM_LOW) || !PAGE_ALIGNED(off) || !PAGE_ALIGNED(addr)) {
        dbg(DBG_PRINT, "(GRADING3D 1)\n");
		return -EINVAL;
	}

	int flag_type = flags & MAP_TYPE;
	KASSERT(!(MAP_PRIVATE == flag_type && MAP_SHARED == flag_type));
	// if ((flag_type == MAP_PRIVATE) && (flag_type == MAP_SHARED)) {
	// 	return -EINVAL;
	// }
	if ((flag_type != MAP_PRIVATE) && (flag_type != MAP_SHARED)) {
        dbg(DBG_PRINT, "(GRADING3D 1)\n");
	...
	if (!(flags & MAP_ANON)) {
		...
		if (flag_type == MAP_PRIVATE && !(file->f_mode & FMODE_READ)) {
			fput(file);
        	dbg(DBG_PRINT, "(GRADING3D 1)\n");
			return -EACCES;
		}
		if (flag_type == MAP_SHARED && (prot & PROT_WRITE)
		...
		KASSERT(!(prot & PROT_WRITE) || (file->f_mode & FMODE_WRITE));
	...
	int res = vmmap_map(curproc->p_vmmap, vnode, ADDR_TO_PN(addr), npages, prot,
...
int do_munmap(void *addr, size_t len) {
	...
	if (vmmap_is_range_empty(curproc->p_vmmap, ADDR_TO_PN(addr), npages)) {
        dbg(DBG_PRINT, "(GRADING3D 2)\n");
		return 0;
	}

	int res = vmmap_remove(curproc->p_vmmap, ADDR_TO_PN(addr), npages);

	tlb_flush_range((uintptr_t) addr, npages);
	pagedir_t *pd = pt_get();
	pt_unmap_range(pd, (uintptr_t) addr,
			(uintptr_t) PN_TO_ADDR(ADDR_TO_PN(addr) + npages));
    dbg(DBG_PRINT, "(GRADING3D 1)\n");
	return res;
}
