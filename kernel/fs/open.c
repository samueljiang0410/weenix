...
int do_open(const char *filename, int oflags) {
	...
	if ((oflags & O_APPEND) == O_APPEND) {
		file->f_mode = file->f_mode | FMODE_APPEND;
		dbg(DBG_PRINT, "(GRADING2B)\n");
	}
	...
	vnode_t *vn;
	int result = open_namev(filename, oflags, &vn, NULL);
	...
	if (S_ISDIR(vn->vn_mode) && (((oflags & O_WRONLY) == O_WRONLY) || ((oflags & O_RDWR) == O_RDWR))) {
		curproc->p_files[fd] = NULL;
		vput(vn);
		fput(file);
		dbg(DBG_PRINT, "(GRADING2B)\n");
	...
	if(filename[strlen(filename)-1] =='/' && !S_ISDIR(vn->vn_mode)){
		vput(vn);
		fput(file);
		curproc->p_files[fd] = NULL;
		dbg(DBG_PRINT, "(GRADING2B)\n");
