...
NULL, .create = NULL, .mknod = NULL, .lookup = NULL, .link = NULL, .unlink =
		NULL, .mkdir = NULL, .rmdir = NULL, .readdir = NULL, .stat =
		special_file_stat, .fillpage = NULL, .dirtypage = NULL, .cleanpage =
		NULL };
...
vget(struct fs *fs, ino_t vno) {
	...
	dbg(DBG_VNREF,
			"vget: 0x%p, 0x%p ino %ld refcount set to %d, nrespages=%d\n", vn,
			vn->vn_fs, (long )vn->vn_vno, vn->vn_refcount, vn->vn_nrespages);
...
static int special_file_read(vnode_t *file, off_t offset, void *buf,
	...
	bytedev_t *chdev = file->vn_cdev;
	int result_read = file->vn_cdev->cd_ops->read(chdev, offset, buf, count);
...
static int special_file_write(vnode_t *file, off_t offset, const void *buf,
	...
	bytedev_t *chdev = file->vn_cdev;
	int result_write = file->vn_cdev->cd_ops->write(chdev, offset, buf, count);
	dbg(DBG_PRINT, "(GRADING2A)\n");
	return result_write;
}
...
static int special_file_mmap(vnode_t *file, vmarea_t *vma, mmobj_t **ret) {
	...
	int result_map = file->vn_cdev->cd_ops->mmap(file, vma, ret);
...
static int special_file_fillpage(vnode_t *file, off_t offset, void *pagebuf) {
	...
	//int result_fillpage = file->vn_cdev->cd_ops->fillpage(file, offset, pagebuf);
	return 0;
}
...
static int special_file_dirtypage(vnode_t *file, off_t offset) {
	...
	//int result_dirtypage = file->vn_cdev->cd_ops->dirtypage(file, offset);
	return 0;
}
...
static int special_file_cleanpage(vnode_t *file, off_t offset, void *pagebuf) {
	...
	//int result_cleanpage = file->vn_cdev->cd_ops->cleanpage(file, offset, pagebuf);
	return 0;
}
